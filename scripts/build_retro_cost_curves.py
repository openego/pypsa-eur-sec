...
# %% ******* (4) SAVE + PLOT  ************************************************
    ...
    if plot:
        ...
        for ct in ["BG", "DE", "SE"]:
            dE = (dE_and_cost.loc[(ct, "tot"), "dE"] * 100)
            cost = dE_and_cost.loc[(ct, "tot"), "cost"]
    ...
    # ---------------
        ...
        p1 = [100, 0]
        p2 = [dE_and_cost.loc[("DE", "tot"), "dE"].loc["0.09"]*100,
              dE_and_cost.loc[("DE", "tot"), "cost"].loc["0.09"]]
        p3 = [dE_and_cost.loc[("DE", "tot"), "dE"].loc["0.2"]*100,
              dE_and_cost.loc[("DE", "tot"), "cost"].loc["0.2"]]
    ...
    #-----------
        ...
        for ct in dE_and_cost.index.levels[0]:
            dE = (dE_and_cost.loc[(ct, "tot"), "dE"] * 100)
            cost = dE_and_cost.loc[(ct, "tot"), "cost"]
            df = pd.concat([dE, cost], axis=1)
            df.columns = ["dE", "cost/m²"]
